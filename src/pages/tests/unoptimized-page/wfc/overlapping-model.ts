import type { NumberCortege as Color, Pattern } from '../../../../wfc/wfc.types';

import { OriginalModel } from './model';

export class OriginalOverlappingModel extends OriginalModel {
		...
		for ( let d = 0; d < 4; d++ ) {
			...
			for ( let t = 0; t < this.T; t++ ) {
				...
				for ( let t2 = 0; t2 < this.T; t2++ ) {
					if ( agrees( this.patterns[t], this.patterns[t2], OriginalModel.dx[d], OriginalModel.dy[d] ) ) {
						list.push( t2 );
					}
				}
